...
void custom_blink(int diod_id, int count, float interval, Timer timer) {
	for (int i = 0; i < count * 2 ;i++) {
		blinkLeds[diod_id].toggle();
		timer.sleep (Timer::FREQUENCY_HZ * (interval));
	}
}
      ...
      while (1)
        {
    	  ...
    	  blinkLeds[0].turnOff();
    	  custom_blink(0, 10, 3, timer);

    	  blinkLeds[1].turnOff();
    	  custom_blink(1, 20, 0.5, timer);
